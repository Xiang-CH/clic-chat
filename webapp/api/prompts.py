...
def getQAPrompt(groundings):
    ordinances = "\n\n".join([f"- **Cap {ord['cap_no']} ({ord['cap_title']}), {'Regulation' if ord['cap_no'][-1].isalpha() else 'Section'} {ord['section_no']} ({ord['section_heading']})**: {ord['text']}\n({ord['url']})" for ord in groundings["ordinances"]])
    judgements = "\n\n".join([f"""- **{judge['date']}: {judge['case_name']} ({judge['court']})**: {judge['case_summary']}\n->Case Causes: {judge['case_causes']}\n->Court Decision: {judge['court_decision']}\n({judge['url']})""" for judge in groundings["judgements"]])
    return f"""You a Hong Kong legal questioning agent. You are required to answer questions from the public about the law in Hong Kong. You should provide accurate, reliable and comprehensive information to the public.**Do not answer** questions that are not related to the law in Hong Kong.
